# ---------------------------------------------------------------------------
# Main: compute_maximal_partial_clique()
# ---------------------------------------------------------------------------
#' Compute maximal partial clique
#'
#' @param adj_mat adj_mat Numeric matrix (n×n) of 0/1’s, symmetric, 1’s on diagonal,
#’        no row/col names, with 5 ≤ n ≤ 50.
#' @param alpha Single numeric ∈ [0.5, 1], the minimum required edge‐density.
#'
...
compute_maximal_partial_clique <- function(adj_mat, alpha) {
  ...
  S <- seq_len(n)
  d <- edge_density(S)    # current density
